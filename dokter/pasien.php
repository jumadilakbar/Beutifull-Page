...
<tbody id="myTable">
  ...
  while ($data=mysql_fetch_array($sql)) {
    ...
    if ($data1['no_rm']==$No_RM) {
      // echo "oke";
      echo '  <a class="btn btn-outline-success" data-toggle="tooltip" data-placement="top" title="Detail" href="?rute=detail&id_periksa='.$data1['id_periksa'].'">
        <i class="fa fa-bars" aria-hidden="true"></i> Detail </a>';
      echo '  <a href="#" class="btn btn-outline-info">Data sudah di masukan</a>';
    }else {
      echo '  <a class="btn btn-outline-success" data-toggle="tooltip" data-placement="top" title="Pemeriksaan" href="?rute=periksa&No_RM='.$data['no_rm'].'">
        <i class="fa fa-bars" aria-hidden="true"></i> Pemeriksaan </a>';
